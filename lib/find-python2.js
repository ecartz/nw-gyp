...
class PythonFinder {
      ...
      checks.push(...[
        {
          ...
          check: () => this.checkCommand('python2')
        },
        {
          before: () => { this.addLog('checking if "python" can be used') },
          check: async () => {
            try {
              return await this.checkCommand('python')
            } catch (err) {
              // If 'python' command fails, try 'python3' as fallback
              this.addLog('- "python" command failed, trying "python3"')
              return this.checkCommand('python3')
            }
          }
        },
        {
  ...
  // being in the $PATH.
  // We try Python 2.7 first, then fall back to Python 3.x
  async checkPyLauncher () {
    // First try Python 2.7
    this.log.verbose(`- executing "${this.pyLauncher}" to get Python 2.7 executable path`)
    try {
      // Try to get the Python 2.7 path using the launcher
      const { execFileSync } = require('child_process')
      const execPath = execFileSync('py', ['-2', '-c', 'import sys; print(sys.executable)'], {
        encoding: 'utf-8',
        stdio: ['ignore', 'pipe', 'ignore']
      }).trim()
      
      if (!execPath) {
        throw new Error('Empty path returned from Python launcher')
      }
      
      this.addLog(`- executable path is "${execPath}"`)
      return this.checkExecPath(execPath)
    } catch (err) {
      this.addLog(`- Python 2.7 not found via py launcher: ${err.message}`)
      
      // Fall back to Python 3.x
      this.addLog('- Trying Python 3.x via py launcher')
      try {
        const { execFileSync } = require('child_process')
        const execPath = execFileSync('py', ['-3', '-c', 'import sys; print(sys.executable)'], {
          encoding: 'utf-8',
          stdio: ['ignore', 'pipe', 'ignore']
        }).trim()
        ...
        this.addLog(`- "${this.pyLauncher}" failed to find Python: ${err.message}`)
        throw err
      }
    }
  }
  ...
  fail () {
    ...
    const info = [
      ...
      'https://github.com/nodejs/nw-gyp#installation',
      '**********************************************************'
    ].join('\n')
