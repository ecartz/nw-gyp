...
async function install (gyp, argv) {
  ...
  // Used to prevent downloading tarball if only new nw.lib is required on Windows.
  ...
  const isLegacy = semver.lt(release.version, '0.11.0-rc1')
  log.verbose('installing legacy version?', isLegacy)

  const isV13 = semver.gt(release.version, '0.13.0-alpha0')

  if (semver.lt(release.version, '0.3.2')) {
    throw new Error('Minimum target version is `0.3.2` or greater. Got: ' + release.version)
  }
  ...
  async function go () {
    ...
    try {
      ...
      //     log.warn(k + ' local checksum ' + contentShasums[k] + ' not match remote ' + expectShasums[k])
      //   }
      // }
    ...
    async function downloadShasums () {
      ...
      if (res.status !== 200) {
        log.warn(`${res.status}  status code downloading checksum`)
        return
      }
      ...
      for (const line of (await res.text()).trim().split('\n')) {
        ...
        // 0035d18e2dcf9aad669b1c7c07319e17abfe3762  ./nw-headers-v0.11.4.tar.gz
    ...
    async function downloadNodeLib () {
      if (isLegacy && (arch !== 'ia32')) {
        throw new Error('nw-gyp only supports 32-bit nw.lib for legacy node versions')
      }
      ...
      if (isV13) {
        const nodeLibPath = path.resolve(dir, 'node.lib')
        log.verbose('streaming ' + arch + ' node.lib to:', nodeLibPath)
        const req = await download(gyp, release[arch].libNodeUrl)

        if (!req || (req.status !== 200)) {
          throw new Error(`${req.status} status code downloading ${arch} node.lib from ${release[arch].libNodeUrl}`)
        }

        await pipeline(
          req.body,
          new ShaSum((_, checksum) => {
            contentShasums[nodeLibPath] = checksum
            log.verbose('content checksum', nodeLibPath, checksum)
          }),
          createWriteStream(nodeLibPath)
        )
      } // V13

      // Since only required nw.lib is downloaded throw error if it is not fetched
      if (res.status !== 200) {
        throw new Error(`${res.status} status code downloading ${name} from ${libUrl}`)
      }
   ...
   * nw-gyp). So what happens is the "nobody" user doesn't have
