  ...
  async function go () {
    ...
    async function downloadNodeLib () {
      ...
      if (isV13) {
        ...
        log.verbose('streaming 32-bit node.lib to:', nodeLibPath)
        const req = await download(gyp, arch === 'x64' ? release.nodeLibUrl64 : release.nodeLibUrl32)

        if (!req || (req.status !== 200)) {
          throw new Error(`${req.status} status code downloading 32-bit node.lib`)
        }
